...
class Onepoint extends React.Component
{
    ...
    OnepointCalcFunction(XL,ErrorApox,Funct)
    {
        ...
        while(data.err>ErrorApox_Answer)
            {
              ...
              fx(x);
              x = fx(x);
              data.err = Math.abs((x-data.xi)/x)*100
              arr=this.state.Arr;
    ...
    handleSubmit(event){
        ...
        const xm = this.OnepointCalcFunction(XL,ErrorApox,Funct)
    ...
    render(){
        return(
          <div>
            ...
            <div>
                <h1 className="h1">&emsp;One-point Iteration&emsp;</h1>
              <label htmlFor='XL' className="XLtext">&emsp;XI :&emsp;</label>
              <input
                className="XLfield"
                name='XL'
                placeholder='XI'
